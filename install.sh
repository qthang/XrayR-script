...
# check root
[[ $EUID -ne 0 ]] && echo -e "${red}Lỗi：${plain} Tập lệnh này phải được chạy với tư cách root！！\n" && exit 1
...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/qthang/XrayR-VietHoa/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}Không phát hiện được phiên bản XrayR，có thể đã vượt quá giới hạn Github API，Vui lòng thử lại sau hoặc chỉ định thủ công phiên bản XrayR để cài đặt${plain}"
            exit 1
        fi
        echo -e "Đã phát hiện phiên bản mới nhất của XrayR：${last_version} , bắt đầu cài đặt"
        wget -q -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/qthang/XrayR-VietHoa/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/qthang/XrayR-Viet-release/releases/download/${last_version}/XrayR-linux-${arch}.zip"
        echo -e "Bắt đầu cài đặt XrayR $1" #$1
    ...
    file="https://github.com/qthang/XrayR-script/raw/master/XrayR.service"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "Cài đặt mới, vui lòng tham khảo hướng dẫn：https://github.com/XrayR-project/XrayR，Định cấu hình nội dung cần thiết"
    else
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}XrayR có thể không khởi động được, vui lòng sử dụng XrayR log để kiểm tra thông tin nhật ký sau này, nếu không khởi động được, định dạng cấu hình có thể đã bị thay đổi, vui lòng vào wiki để kiểm tra：https://github.com/XrayR-project/XrayR/wiki${plain}"
        fi
    fi
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/qthang/XrayR-script/master/XrayR.sh
    chmod +x /usr/bin/XrayR
        ln -s /usr/bin/XrayR /usr/bin/xrayr # Chữ thường tương thích
    chmod +x /usr/bin/xrayr
    echo -e ""
    echo "------------[QThang.net Việt hóa]------------"
