...
#    fi
    ...
    if [[ $? == 0 ]]; then
        echo -e "${green}Cập nhật hoàn tất, XrayR đã được khởi động lại tự động, vui lòng sử dụng XrayR log để xem nhật ký đang chạy ${plain}"
        exit
    fi
...
uninstall() {
    ...
    echo ""
    echo -e "Gỡ cài đặt thành công, nếu bạn muốn xóa tập lệnh này, hãy chạy ${green}rm /usr/bin/XrayR -f${plain} sau khi thoát tập lệnh để xóa"
    echo ""
...
start() {
    ...
    if [[ $? == 0 ]]; then
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}XrayR có thể không khởi động được, vui lòng sử dụng XrayR log để xem thông tin nhật ký ${plain}"
        fi
    fi
...
stop() {
    ...
    if [[ $? == 1 ]]; then
        ...
        echo -e "${red}XrayR không dừng được, có thể do thời gian dừng vượt quá hai giây, vui lòng kiểm tra thông tin nhật ký ${plain}"
    fi
...
show_usage() {
    echo ""
    echo "------------[4GFree.QThang.net việt hóa]------------"
...
————————————————
  ...
  ${green}8.${plain} Xem nhật ký XrayR log
————————————————
